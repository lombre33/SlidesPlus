// variable-format.js — formatage nombre/date d'une variable résolue - aucune dépendance externe : Intl.NumberFormat/Intl.DateTimeFormat couvrent tout sauf
// l'écriture d'un nombre en toutes lettres (pas d'équivalent Intl), écrite à la main ci-dessous (orthographe classique "vingt et un", pas la réforme de 1990).
// Copié tel quel depuis publipostageGrist (js/variable-format.js) — module déjà autonome, aucune adaptation nécessaire pour être partagé entre widgets.
// I18n est optionnel (détecté via typeof) : sans lui, tout retombe simplement sur le français.
